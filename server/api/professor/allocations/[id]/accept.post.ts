...
export default defineEventHandler(async (event) => {
  ...
  // ensure this allocation belongs to this professor
  const { data: alloc, error: aErr } = await supabase
    .from('jury_allocations')
    .select('id, professor_profile_id, status, thesis_request_id, responded_at, invited_at, professor_profiles!inner(profile_id)')
    .eq('id', id)
    .single()
